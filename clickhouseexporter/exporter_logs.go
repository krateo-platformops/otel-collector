...
// createSchema (re)creates the database and logs table. It is idempotent
...
// the exporter kept running (see isTableMissing / pushLogsData).
func (e *logsExporter) createSchema(ctx context.Context) error {
...
func (e *logsExporter) pushLogsData(ctx context.Context, ld plog.Logs) error {
	if err := e.doPushLogsData(ctx, ld); err != nil {
		...
		if e.cfg.shouldCreateSchema() && isTableMissing(err) {
			e.logger.Warn("clickhouse logs table missing on insert; recreating schema and retrying", zap.Error(err))
			if createErr := e.createSchema(ctx); createErr != nil {
				return errors.Join(err, createErr)
			}
			return e.doPushLogsData(ctx, ld)
		}
		return err
	}
	return nil
...
func (e *logsExporter) doPushLogsData(ctx context.Context, ld plog.Logs) error {
	start := time.Now()
