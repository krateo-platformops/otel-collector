...
// createSchema (re)creates the database and traces tables. It is idempotent
...
// the exporter kept running (see isTableMissing / pushTraceData).
func (e *tracesExporter) createSchema(ctx context.Context) error {
...
func (e *tracesExporter) pushTraceData(ctx context.Context, td ptrace.Traces) error {
	if err := e.doPushTraceData(ctx, td); err != nil {
		...
		if e.cfg.shouldCreateSchema() && isTableMissing(err) {
			e.logger.Warn("clickhouse traces table missing on insert; recreating schema and retrying", zap.Error(err))
			if createErr := e.createSchema(ctx); createErr != nil {
				return errors.Join(err, createErr)
			}
			return e.doPushTraceData(ctx, td)
		}
		return err
	}
	return nil
...
func (e *tracesExporter) doPushTraceData(ctx context.Context, td ptrace.Traces) error {
	start := time.Now()
