...
(`processor.go:54-57`) — there is **no** kubeconfig fallback. The collector will fail to start the
`compositionresolver` processor outside a pod (no ServiceAccount token / API host env). This is by
design (it's a cluster-internal ingestion component) but means you cannot exercise the resolve path
with a local `otelcol-krateo` against a remote cluster.
...
## 8. Hard version pinning across OCB / contrib / client-go
...
and `k8s.io/client-go v0.32.0` (`go.mod:6-12`). These move together: bumping contrib without
bumping OCB and the custom processor's `go.mod` will break the OCB build (collector API drift
between minor versions). Treat the version set as one unit.
...
`builder-config.yaml`'s `extensions:` — only `healthcheckextension`, `builder-config.yaml:26-27`).
